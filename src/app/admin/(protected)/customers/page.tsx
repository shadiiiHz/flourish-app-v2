...
/** Converts a <input type="date"> value (YYYY-MM-DD, no timezone) into an ISO datetime string at UTC midnight. */
function dateInputToIso(value: string): string {
  return new Date(`${value}T00:00:00.000Z`).toISOString();
}

/** Converts an ISO datetime string back into a <input type="date"> value — the inverse of dateInputToIso. */
...
function AdminCustomersPage() {
    ...
    onSubmit: async (values, helpers) => {
      try {
        await adminCreateCustomer({
          ...
          birthDate: values.birthDate ? dateInputToIso(values.birthDate) : undefined,
        });
        helpers.resetForm();
        setCreateOpen(false);
        load();
  ...
  const editFormik = useFormik<NewCustomerForm>({
    initialValues: NEW_CUSTOMER_FORM,
    ...
    onSubmit: async (values, helpers) => {
      ...
      try {
        const updated = await adminUpdateCustomer(editingCustomer.id, {
          phone: normalizeDigits(values.phone),
          firstName: values.firstName || undefined,
          lastName: values.lastName || undefined,
          email: values.email || undefined,
          birthDate: values.birthDate ? dateInputToIso(values.birthDate) : null,
        });
        setEditingCustomer(null);
        setDetail((prev) => (prev && prev.id === updated.id ? { ...prev, ...updated } : prev));
        load();
      } catch (err) {
        helpers.setStatus(err instanceof ApiError ? err.message : "خطا در ویرایش مشتری");
      } finally {
        helpers.setSubmitting(false);
      }
    },
  ...
  const openEdit = (customer: AdminCustomer) => {
    ...
    editFormik.resetForm({
      values: {
        ...
        birthDate: isoToDateInput(customer.birthDate),
      },
    });
  };
  ...
  const columns = useMemo<GridColDef<AdminCustomer>[]>(
    () => [
      ...
      },
      {
        ...
        headerName: "منبع",
        width: 160,
        valueFormatter: (_, row) => CUSTOMER_SOURCE_LABELS[row.source],
        renderCell: ({ row }) => (
          <span
          ...
          >
            {CUSTOMER_SOURCE_LABELS[row.source]}
          </span>
        ),
      },
      {
  ...
  return (
    <div>
      ...
      <Dialog
        ...
        dir="rtl"
      >
        <DialogTitle className="font-display text-cocoa-900">ویرایش مشتری</DialogTitle>
        <DialogContent dividers>
          <form onSubmit={editFormik.handleSubmit} className="flex flex-col gap-3">
            <div>
              <label className="mb-1 block text-xs font-semibold text-cocoa-600">
                شماره موبایل
              </label>
              <input
                ...
                onChange={(e) =>
                  editFormik.setFieldValue("phone", normalizeDigits(e.target.value))
                }
            ...
            {editFormik.status && (
              <p className="text-xs font-semibold text-danger-500">{editFormik.status}</p>
            )}
            ...
            <button
              ...
              className="mt-1 rounded-full bg-sand-500 px-5 py-2.5 text-sm font-bold text-white shadow-[0_10px_20px_-8px_rgba(164,72,25,0.6)] transition-transform hover:scale-[1.02] active:scale-95 disabled:opacity-60"
            >
              {editFormik.isSubmitting ? "در حال ذخیره…" : "ذخیره تغییرات"}
            </button>
          </form>
        </DialogContent>
      </Dialog>
